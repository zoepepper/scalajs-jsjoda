...
object LocalDateTime {
  ...
  def ofEpochSecond(epochSecond: Long, nanoOfSecond: Int, offset: ZoneOffset): LocalDateTime =
    LocalDateTimeF.ofEpochSecond(epochSecond.toDouble, nanoOfSecond, offset)
...
class LocalDateTime protected[time](f: LocalDateTimeF) extends Wraps(f) with ChronoLocalDateTime[LocalDate] {
  ...
  def minus(amount: Long, unit: TemporalUnit): LocalDateTime = f.minus(amount.toDouble, unit)
  def minusYears(yearsToSubtract: Long): LocalDateTime = f.minusYears(yearsToSubtract.toDouble)
  def minusMonths(monthsToSubtract: Long): LocalDateTime = f.minusMonths(monthsToSubtract.toDouble)
  def minusWeeks(weeksToSubtract: Long): LocalDateTime = f.minusWeeks(weeksToSubtract.toDouble)
  def minusDays(daysToSubtract: Long): LocalDateTime = f.minusDays(daysToSubtract.toDouble)
  def minusHours(hoursToSubtract: Long): LocalDateTime = f.minusHours(hoursToSubtract.toDouble)
  def minusMinutes(minutesToSubtract: Long): LocalDateTime = f.minusMinutes(minutesToSubtract.toDouble)
  def minusSeconds(secondsToSubtract: Long): LocalDateTime = f.minusSeconds(secondsToSubtract.toDouble)
  def minusNanos(nanosToSubtract: Long): LocalDateTime = f.minusNanos(nanosToSubtract.toDouble)
  def plus(amount: TemporalAmount): LocalDateTime = f.plus(amount)
  def plus(amount: Long, unit: TemporalUnit): LocalDateTime = f.plus(amount.toDouble, unit)
  def plusYears(yearsToAdd: Long): LocalDateTime = f.plusYears(yearsToAdd.toDouble)
  def plusMonths(monthsToAdd: Long): LocalDateTime = f.plusMonths(monthsToAdd.toDouble)
  def plusWeeks(weeksToAdd: Long): LocalDateTime = f.plusWeeks(weeksToAdd.toDouble)
  def plusDays(daysToAdd: Long): LocalDateTime = f.plusDays(daysToAdd.toDouble)
  def plusHours(hoursToAdd: Long): LocalDateTime = f.plusHours(hoursToAdd.toDouble)
  def plusMinutes(minutesToAdd: Long): LocalDateTime = f.plusMinutes(minutesToAdd.toDouble)
  def plusSeconds(secondsToAdd: Long): LocalDateTime = f.plusSeconds(secondsToAdd.toDouble)
  def plusNanos(nanosToAdd: Long): LocalDateTime = f.plusNanos(nanosToAdd.toDouble)
  ...
  def `with`(field: TemporalField, newValue: Long): LocalDateTime = f.`with`(field, newValue.toDouble)
