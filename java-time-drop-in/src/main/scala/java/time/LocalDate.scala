...
object LocalDate {
  ...
  def ofEpochDay(epochDay: Long): LocalDate = LocalDateF.ofEpochDay(epochDay)
...
class LocalDate protected[time](f: LocalDateF) extends Wraps(f) with ChronoLocalDate {
  ...
  def minusDays(daysToSubtract: Long): LocalDate = f.minusDays(daysToSubtract)
  def minusMonths(monthsToSubtract: Long): LocalDate = f.minusMonths(monthsToSubtract)
  def minusWeeks(weeksToSubtract: Long): LocalDate = f.minusWeeks(weeksToSubtract)
  def minusYears(yearsToSubtract: Long): LocalDate = f.minusYears(yearsToSubtract)
