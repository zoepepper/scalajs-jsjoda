...
object LocalTime {
  ...
  def ofNanoOfDay(nanoOfDay: Long): LocalTime = LocalTimeF.ofNanoOfDay(nanoOfDay.toDouble)
  def ofSecondOfDay(secondOfDay: Long): LocalTime = LocalTimeF.ofSecondOfDay(secondOfDay.toDouble)
...
class LocalTime protected[time](f: LocalTimeF) extends Wraps(f) with Comparable[LocalTime]
  ...
  def minus(amount: Long, unit: TemporalUnit): LocalTime = f.minus(amount.toDouble, unit)
  def minusHours(hoursToSubtract: Long): LocalTime = f.minusHours(hoursToSubtract.toDouble)
  def minusMinutes(minutesToSubtract: Long): LocalTime = f.minusMinutes(minutesToSubtract.toDouble)
  def minusSeconds(secondsToSubtract: Long): LocalTime = f.minusSeconds(secondsToSubtract.toDouble)
  def minusNanos(nanosToSubtract: Long): LocalTime = f.minusNanos(nanosToSubtract.toDouble)
  def plus(amount: TemporalAmount): LocalTime = f.plus(amount)
  def plus(amount: Long, unit: TemporalUnit): LocalTime = f.plus(amount.toDouble, unit)
  def plusHours(hoursToAdd: Long): LocalTime = f.plusHours(hoursToAdd.toDouble)
  def plusMinutes(minutesToAdd: Long): LocalTime = f.plusMinutes(minutesToAdd.toDouble)
  def plusSeconds(secondsToAdd: Long): LocalTime = f.plusSeconds(secondsToAdd.toDouble)
  def plusNanos(nanosToAdd: Long): LocalTime = f.plusNanos(nanosToAdd.toDouble)
  ...
  def `with`(field: TemporalField, newValue: Long): LocalTime = f.`with`(field, newValue.toDouble)
