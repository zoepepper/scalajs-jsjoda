...
class YearMonth protected[time](f: YearMonthF) extends Wraps(f) with Comparable[YearMonth]
  ...
  def minus(amountToSubtract: Long, unit: TemporalUnit): YearMonth = f.minus(amountToSubtract.toDouble, unit)
  def minus(amountToSubtract: TemporalAmount): YearMonth = f.minus(amountToSubtract)
  def minusMonths(monthsToSubtract: Long): YearMonth = f.minusMonths(monthsToSubtract.toDouble)
  def minusYears(yearsToSubtract: Long): YearMonth = f.minusYears(yearsToSubtract.toDouble)
  def plus(amountToAdd: Long, unit: TemporalUnit): YearMonth = f.plus(amountToAdd.toDouble, unit)
  def plus(amountToAdd: TemporalAmount): YearMonth = f.plus(amountToAdd)
  def plusMonths(monthsToAdd: Long): YearMonth = f.plusMonths(monthsToAdd.toDouble)
  def plusYears(yearsToAdd: Long): YearMonth = f.plusYears(yearsToAdd.toDouble)
  def `with`(adjuster: TemporalAdjuster): YearMonth = f.`with`(adjuster)
  def `with`(field: TemporalField, newValue: Long): YearMonth = f.`with`(field, newValue.toDouble)
